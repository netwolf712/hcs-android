...
public class SNTPClient {
    ...
    public boolean requestTime(String host, int timeout) {
        ...
        try {
            ...
            //             = (2 * skew)/2 = skew
            KLog.d(TAG, "Request time form ntp server success, " + address + " ,roundTripTime: " + roundTripTime);
            if (DBG) {
                KLog.d(TAG, "round trip: " + roundTripTime + "ms, " +
...
//            }
            e.printStackTrace();
            KLog.e(TAG, "Request time from ntp server failed ,msg: " + e.getMessage());
            return false;
        } finally {
            if (socket != null) {
                socket.close();
